...
const TargetGroupsForm: React.FC<IFormStepProps> = ({ courseId }) => {
  ...
  const handleSelectTargetGroup = (id: number) => {
    if (!courseTargetGroups.some((group) => group.id === id)) {
      ...
      courseService.linkTargetGroupToCourse(token, courseId, id)
  ...
  const handleAddNewTargetGroup = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        courses: [courseId], // Link to the current course
      };
      const response = await courseService.createTargetGroup(token, payload);
      const newGroup = response.data;
      console.log("New target group created:", newGroup);

      setTargetGroups([...targetGroups, newGroup]); // Update global list
      setCourseTargetGroups([...courseTargetGroups, newGroup]); // Associate with course
